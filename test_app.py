...
def test_build_answer_code():
    """Tests that each answer code has a valid format, like q5b, q4c etc"""
...
def test_get_user_mood_happy():
    """Tests if the correct mood is being selected from the respective user answer codes. Mood=Happy"""
    answers = ['q1a', 'q4a', 'q7a', 'q9a']
    assert get_user_mood(answers, answer_tree) == 'happy'


def test_get_user_mood_inspired():
    """Tests if the correct mood is being selected from the respective user answer codes. Mood=Inspired"""
    answers = ['q2b', 'q5a', 'q7c', 'q8b']
    assert get_user_mood(answers, answer_tree) == 'inspired'


def test_collective_book_list():
    """Tests if the total number of scraped books is 5 and they each contain 6 elements."""
    five_test_books = [
...
def test_three_highest_ratings():
    """Tests that the code is sorting the book ratings from highest to lowest and keeping the top 3."""
